...
export function Perfil() {
  const {
    actions: { ProximaEtapa },
  } = userStore();
  const {
    actions: { adicionarNome, adicionarEmail, adicionarTelefone },
  } = useFormContext();

  function formResposta({ nome, email, telefone }: formProps) {
    adicionarNome(nome);
    ...
    <div className={s.Container}>
      <div className={s.titulo}>
        <Header
          ...
          subtitulo="Informe seu nome, email telefone"
        />
      </div>
      ...
      <form onSubmit={handleSubmit(formResposta)}>
        ...
        <span>
          Telefone {errors.telefone?.message && `${errors.telefone?.message}`}
        </span>
